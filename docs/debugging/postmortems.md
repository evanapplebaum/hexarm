...
Consolidated log of hardware/software incidents encountered while building hexarm — each one root-caused, not just patched. Comprehensive chronology for incidents 1–2 lives in [`servo-comms-debug-log.md`](servo-comms-debug-log.md); incident 3's original handoff doc lives at `docs/context mds/lerobot-calibration-wrap-around.md`.
...
**Root cause:** A kernel serial console + `serial-getty@ttyAMA0.service` were still attached to the same UART used for the servo bus. The getty silently consumed incoming bytes before pyserial could read them, wrote login-prompt characters onto the TX line, and held the port at its console baud rate at the kernel level. This explained every symptom from pass one — the "always missing byte 1" pattern was the getty grabbing bytes from the line randomly, and "retry helps" was coincidence, not RC warm-up. On top of that, pass one's resync-parser "fix" turned out to have its own bug: reading `length+1` bytes to recover a dropped header silently stole a byte that `ping()`'s second, chained read still needed — a second, independent bug hiding behind the first.
...
**Lesson:** A theory that explains every observed symptom can still be wrong — the fix is a falsifying experiment, not more confirming evidence. The loopback test collapsed the whole framing-error narrative in one command. `stty` reporting a baud rate you never set is a red flag that something else owns the port; `dmesg | grep -iE "uart|serial|tty"` should be one of the first checks. Monkey-patches to shared SDK internals need to be audited across *every* call site, not just the one under test — a fix for a single-step read broke a multi-step one.
...
- **Attempt 2 — finger-hole attachments:** printed two finger-hole mounts that attach to the existing gripper bars, intended to give a more natural grip while leaving fingers free for the other joints. Problems:
...
**Root cause:** "Limp-mode" still required significant torque, and PLA wasn't stiff enough to transmit that torque through a second lever arm without bending or snapping. 

**Fix:** Crank mechanism: rotated the servo mount and attached a crank to the shaft plate, sacrificing leader-follower link symmetry for precise, stable control of the gripper. Since the crank had no physical joint limit, normalized values were used to map a wide range of crank positions to the constrained follower claw limits. This change additionally allowed for more precise control of the follower. 

**Lesson:** When working with brittle materials such as PLA, consider which components will be subject to higher stress **before manufacturing**, especially when precision is necessary. 

---

## 5. Claw Too Small — Dropping Grasped Objects

**Dates:** started 2026-07-27 · **Severity:** Impactful but non-blocking

**Symptom:** The follower claw's jaw opening/contact geometry is undersized for the objects being grasped during teleop — grip doesn't fully secure the object, and it either snaps or slips mid-manipulation.

**Root cause:** 

**Fix:** To allow for flexibility when grasping objects and avoid snapping, two TPU claw pads were designed to fit onto the ends of the gripper arms. 

**Lesson:** When working with claw-like components, leverage materials with specific properties based on the specific payloads being manipulated. Specifically, flexible materials like TPU can be used to make a claw forgiving without sacrificing gripping capability. In sum, when mechanical precision is difficult or varied, flexible materials improve adaptability and distribute load.

---

## 6. Arm Base Instability — Bending/Tipping Under Load

**Dates:** during CAD/build assembly, resolved by 2026-08-09 · **Severity:** Blocking (assembly)

**Symptom:** The free-standing arm base bent and the arm tipped/fell over during assembly and testing.

**Investigation:** The base had been sized to support the arm's static weight while sitting still, not the tipping moments generated once the arm moves — the shoulder/base joint applies a lever-arm load that a free-standing footprint has to resist entirely through its own stiffness and mass.
...
**Lesson:** Inertia is an important design consideration. Where possible, use fasteners (screws, bolts, clamps) to improve rigidity instead of relying on friction, and design around worst-case conditions.

---

## 7. Servo-Mount Joint Bending — PLA Screw Points Undersized

**Dates:** during CAD/build assembly, resolved by 2026-08-09 · **Severity:** Moderate

**Symptom:** Printed links bent under load, specifically at the joints, at the point where the PLA part screws into the servo horn.
...
**Lesson:** Fastener attachment points on a printed part are a distinct sizing problem from the part's general wall thickness — treat them as their own local stress concentration and reinforce them directly. As a general rule of thumb, design attachment points with care, using ANSYS to determine high-stress areas if not apparent.

---

## 8. Phantom Episodes — Silent Data Loss From a Resolution Mismatch on `--resume`

**Dates:** 2026-08-10 · **Severity:** High (unrecoverable data loss)
...
**Dates:** 2026-08-11 to 2026-08-13 · **Severity:** Blocking (required a physical power cycle both times to free ram)
...
**Dates:** First run (2026-08-19) · **Severity:** High (near-miss, no actual damage)

**Symptom:** First physical run of the trained policy (`act_pick_and_place_v2`, 25,000-step checkpoint, via `run_policy.py`'s dead-man's-switch mode) completed the pick-and-place task successfully, but with thin real-world margins in two places: (1) during the pick, the claw would sometimes graze the block while closing around it — if contact had been harder, it risked breaking the claw; (2) during the place, the trajectory toward the bowl would often pass just over the bowl's top lip, with little clearance to spare.
