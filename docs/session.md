...
- **Training-time decision: cloud, not the Jetson.** Benchmarked real ACT training step time on this hardware (1.29s/step measured) — a realistic 5–10 epoch run projects to ~6.4–12.7 hours locally vs. ~30–90 minutes on a rented RTX 4090 (well under $1) or HF Jobs. Given the Jetson also runs the robot itself and multiple training iterations are likely, train in the cloud. `lerobot-train ... --job.target=a10g-large` (HF Jobs, needs `hf auth login` once) is the lowest-friction path since it's built into the same `lerobot-train` CLI already in use.
