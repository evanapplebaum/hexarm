...
- No swap configured on the Jetson by default — local GPU training (dataloader prefetch + desktop GUI baseline) can push the unified 7.3GB memory pool to ~98% full and hard-freeze the board with zero warning, no panic, no OOM-kill message. `systemd-oomd` doesn't help here — this kernel has no `/proc/pressure/memory`, so the service silently never arms. Fixed via `zram-tools` (`ALGO=zstd`, not the package default `lz4` — unsupported on this kernel's zram driver — at `PERCENT=50`). See postmortems.md #9.
...
### Current state
- **All 50 dataset episodes recorded (2026-08-10) — `hexarm/pick_and_place_v2`, 640×400 @ 30fps, ~28.3k frames.** Verified structurally sound (info.json/data/video/episode-metadata all agree, every episode's boundary frames decode correctly through the real `LeRobotDataset` load path) and visually reviewed via a generated thumbnail-grid QA page (start/mid/end frames, both cameras, all 50 episodes). **Dataset recording is done — policy training is next.** One real incident this session: a resolution-speedup fix interacted badly with `--resume` on the *old* `hexarm/pick_and_place` dataset and silently cost 2 unrecoverable episodes there (root-caused, fixed, full writeup in `postmortems.md` #8) — the original dataset's 3 valid episodes are untouched and were kept, just not used for the 50-episode set. Also fixed along the way: arms weren't returning to neutral after a session's *last* episode, an image-writer thread-starvation bug that made ENTER appear to hang, unretried servo writes crashing on a single dropped packet (`num_retry=3` added across 5 scripts), `--resume`'s missing default root, and a ~3.4× video-encode speedup (1280×800 → 640×400 downscale, resolution was the actual bottleneck, not codec — confirmed via direct benchmark before committing to the fix).
- **Training is done — 25,000 steps, completed locally overnight, zero issues.** A local `lerobot-train` benchmark hard-froze the board twice before being fixed (RAM exhaustion, no swap — full writeup in `postmortems.md` #9, fix was `zram-tools` at `zstd`/`PERCENT=50`). With that in place, the real run (`tmux`-protected against SSH drops) finished all 25,000 steps in ~6h12m with no crash. 5 checkpoints saved under `outputs/train/act_pick_and_place_v2/checkpoints/` (`last` → `025000`).
- **Checkpoints evaluated — final one confirmed best.** No training-loss history survived (wandb was off, tmux scrollback rolled past it), so ran a direct eval instead: L1 loss 0.1742→0.0942 across the 5 checkpoints, clean monotonic improvement, no overfitting.
- **Next up: run it on the arm.** `software/control/run_policy.py` is written — dead-man's-switch gated (hold SPACE to let the policy act, release freezes instantly, same pattern as `go_neutral.py --diagnostic`), follower-only (no leader arm involved). Verified as far as possible without hardware (imports, real checkpoint load, feature shapes) but never actually run against the physical arm yet. Run: `source /data/lerobot-env/bin/activate && cd /data/hexarm && python software/control/run_policy.py`. See `context.md` session-14 log.
...
Done since last update: ~~Record 50-episode pick-and-place dataset~~ (2026-08-10, `hexarm/pick_and_place_v2`), ~~Fix record_dataset.py hardening (neutral-return, image-writer threads, num_retry, --resume root)~~ (2026-08-10), ~~Root-cause and fix phantom-episode data-loss bug~~ (2026-08-10, postmortems.md #8), ~~Benchmark Jetson vs. cloud training time, decide on cloud~~ (2026-08-10).
