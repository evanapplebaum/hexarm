...
- **Python venv (raw serial diagnostics):** `/data/hexarm/.venv` — separate from the LeRobot env, only needs `pyserial`. See `docs/session.md`'s "Python environments" table for which to use when.
...
| Run trained policy on hardware | ⏳ Todo — next up. `software/control/run_policy.py` written and verified (imports, actual checkpoint load onto CUDA, feature-name/shape assumptions all confirmed) but never run against the physical arm — that's the one remaining unknown. See context.md session-14 log entry for the full design writeup. |
